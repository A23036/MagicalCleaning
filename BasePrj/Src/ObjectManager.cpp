...
namespace {
	...
	const int distQNum = 4; // âÊñ ï™äÑç≈ëÂêî 
	struct DrawObject {
		GameObject* object;
		int order;
		float distQ[distQNum]; // éãì_Ç©ÇÁÇÃãóó£ÇÃÇQèÊ[âÊñ ï™äÑç≈ëÂï™] 
		bool visible;
	...
	bool needSortUpdate;
	bool needSortDraw;
	int drawTimes = 1; // ï`âÊâÒêî(âÊñ ï™äÑêîÇ∆ìØÇ∂) 
	int drawCounter; // ï`âÊÉJÉEÉìÉ^Å[ÅiâΩâÊñ ñ⁄ÇÃï`âÊÇ©Åj 
};
...
{
	// drawTimesâÒÅAï`âÊèàóùÇçsÇ§
	for (drawCounter = 0; drawCounter < drawTimes; drawCounter++)//drawTimesâÒêîï`âÊèàóùÇÇ∑ÇÈ 
	{
		if (needSortDraw) {
			// É\Å[Égèàóù ÉLÅ[ÇP:óDêÊìxÇÃç~èá ÉLÅ[ÇQ:éãì_Ç©ÇÁÇÃãóó£ÇÃç~èá
